...
export async function GET(request:Request){
 ...
 if(!id)return ensureSession(request,reply({ready:isGeminiReady()&&!!process.env.DATABASE_URL,signedIn:true}));
...
RESEARCH: ${researched.text}`,false,10000);
 const result=resultSchema.parse(parseJson(shaped.text));const works=result.works.map(w=>({...w,url:requireSource(sources,w.source).url}));
 const slot=cornerIndex(seed.format);if(works[slot].title.toLowerCase().trim()!==seed.title.toLowerCase().trim())throw new Error('Seed was changed');works[slot]={...works[slot],title:seed.title,creator:seed.creator,format:seed.format,url:requireSource(lookup.sources,seed.source).url};
