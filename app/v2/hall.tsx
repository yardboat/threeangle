...
export default function Hall({initialId,startWithTitle=false}:{initialId?:string;startWithTitle?:boolean}){
 ...
 useEffect(()=>{
  if(stage!=='welcome'||paused||reduced)return;
  const element=world.current;let frame=0;const scroll=()=>{if(frame)return;frame=requestAnimationFrame(()=>{world.current?.style.setProperty('--hall-scroll',Math.min(window.scrollY*.025,12)+'px');frame=0;});};
 ...
 const revisit=(t:Topic)=>{if(topic&&topic.id!==t.id)setPrevious(topic);openTopic(t);};
